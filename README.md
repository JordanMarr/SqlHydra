...
generated column at every write site. Instead, the generator emits a second record for the table,
`{table}_write`: the read record minus those columns, marked `IWriteOf<{table}>`, and a `ToWrite()`
on the read record that projects onto it:

```fsharp
type invoice = { id: int; seq: int; price: decimal; tax: decimal }
    member this.ToWrite() : invoice_write = { seq = this.seq; price = this.price }

and invoice_write = { seq: int; price: decimal }
    interface IWriteOf<invoice>
```

`writeEntity` and `writeEntities` take it where `entity` and `entities` take the read record; the
loop variable stays the read record, so `where`, `getId`, `includeColumn` and `excludeColumn`
select over it as before:
...
updateTask db {
    ...
    writeEntity { row.ToWrite() with price = 20m }   // row: a sales.invoice read back earlier
    where (i.id = row.id)
}
```
...
`entity` is untouched: it still names every column, and on such a table PostgreSQL still rejects
it, as it does today; so does `set i.tax 5m`. `GENERATED BY DEFAULT AS IDENTITY` is writable, so
it stays on the write record. A table with no read-only column gets no write record, nor does one
where every column is read-only, since a record cannot be empty. `onConflict ... doUpdate i.tax`
still selects over the read record, so a read-only column can reach that `SET`; the database
rejects it, as before. Detection is PostgreSQL-only for now; the write record and `writeEntity` are not.
