// This code was generated by `SqlHydra.SqlServer` -- v0.210.0.0.
...
module dbo =
    ...
    type HydraReader(reader: Microsoft.Data.SqlClient.SqlDataReader) =
        ...
        static member GetPrimitiveReader(t: System.Type, reader: Microsoft.Data.SqlClient.SqlDataReader) =
            if t = typedefof<System.Guid> then Some(reader.GetGuid >> box)
            else if t = typedefof<bool> then Some(reader.GetBoolean >> box)
            else if t = typedefof<int> then Some(reader.GetInt32 >> box)
            else if t = typedefof<int64> then Some(reader.GetInt64 >> box)
            else if t = typedefof<int16> then Some(reader.GetInt16 >> box)
            else if t = typedefof<byte> then Some(reader.GetByte >> box)
            else if t = typedefof<double> then Some(reader.GetDouble >> box)
            else if t = typedefof<System.Single> then Some(reader.GetFloat >> box)
            else if t = typedefof<decimal> then Some(reader.GetDecimal >> box)
            else if t = typedefof<string> then Some(reader.GetString >> box)
            else if t = typedefof<System.DateTimeOffset> then Some(reader.GetDateTimeOffset >> box)
            else if t = typedefof<System.DateTime> then Some(reader.GetDateTime >> box)
            else if t = typedefof<System.TimeSpan> then Some(reader.GetTimeSpan >> box)
            else if t = typedefof<byte []> then Some(reader.GetValue >> box)
            else if t = typedefof<obj> then Some(reader.GetValue >> box)
            else None
