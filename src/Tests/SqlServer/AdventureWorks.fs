// This code was generated by `SqlHydra.SqlServer` -- v0.220.0.0.
...
module SalesLT =
    ...
    type HydraReader(reader: Microsoft.Data.SqlClient.SqlDataReader) =
        ...
        static member private GetPrimitiveReader(t: System.Type, reader: Microsoft.Data.SqlClient.SqlDataReader, isOpt: bool) =
            ...
            if t = typedefof<System.Guid> then Some(reader.GetGuid >> wrap)
            else if t = typedefof<bool> then Some(reader.GetBoolean >> wrap)
            else if t = typedefof<int> then Some(reader.GetInt32 >> wrap)
            else if t = typedefof<int64> then Some(reader.GetInt64 >> wrap)
            else if t = typedefof<int16> then Some(reader.GetInt16 >> wrap)
            else if t = typedefof<byte> then Some(reader.GetByte >> wrap)
            else if t = typedefof<double> then Some(reader.GetDouble >> wrap)
            else if t = typedefof<System.Single> then Some(reader.GetFloat >> wrap)
            else if t = typedefof<decimal> then Some(reader.GetDecimal >> wrap)
            else if t = typedefof<string> then Some(reader.GetString >> wrap)
            else if t = typedefof<System.DateTimeOffset> then Some(reader.GetDateTimeOffset >> wrap)
            else if t = typedefof<System.DateTime> then Some(reader.GetDateTime >> wrap)
            else if t = typedefof<System.TimeSpan> then Some(reader.GetTimeSpan >> wrap)
            else if t = typedefof<byte []> then Some(reader.GetValue >> wrap)
            else if t = typedefof<obj> then Some(reader.GetValue >> wrap)
            else None
